...
const menuItemStyles = (verticalNavOptions, theme) => {
  ...
  const greyText = '#6B7280' // medium grey
  const greyIcon = '#6B7280' // same for icons
  const greyHover = '#2B3380' // light grey for hover background
  ...
  return {
    root: ({ level }) => ({
      ...
      [`&:not(.${menuClasses.subMenuRoot}) > .${menuClasses.button}.${menuClasses.active}`]: {
        ...
        [`& .${menuClasses.icon}`]: { color: greyIcon }
      }
    }),
    button: ({ level, active }) => ({
      paddingBlock: '8px',
    ...
    icon: ({ level }) => ({
      color: greyIcon,
      transition: `margin-inline-end ${transitionDuration}ms ease-in-out`,
      ...(level === 0 && { fontSize: '1.375rem', marginInlineEnd: theme.spacing(2) }),
      ...(level > 0 && { fontSize: '0.75rem', marginInlineEnd: theme.spacing(3.5) }),
      ...(collapsedNotHovered && { marginInlineEnd: 0 }),
      ...(popoutCollapsed && level > 0 && { marginInlineEnd: theme.spacing(2) }),
    ...
    subMenuExpandIcon: {
      ...
      marginInlineStart: theme.spacing(2),
      color: greyIcon,
      '& i, & svg': { fontSize: 'inherit', color: greyIcon }
    }
  }
}
