  ...
  return (
    <>
      {data.map((slide, index) => {
        return (
          <div key={index} className={classnames('keen-slider__slide p-6 pbe-3 is-full')}>
            <Typography variant='h5' className='mbe-0.5 text-black'>
              Website Analytics
            </Typography>
            <Typography variant='subtitle2' className='mbe-3 text-gray-700'>
              Total 28.5% Conversion Rate
            </Typography>
            <Grid container spacing={4} className='relative'>
              <Grid size={{ xs: 12, sm: 8 }} className='order-2 sm:order-1'>
                <div className='flex flex-col gap-4 sm:plb-6'>
                  <Typography className='font-medium text-black'>{slide.title}</Typography>
                  <Grid container spacing={4}>
                    {Object.keys(slide.details).map((key, index) => {
                      return (
                        <Grid key={index} size={{ xs: 6 }}>
                          <div className='flex items-center gap-0.5'>
                            <CustomAvatar
                              color='primary'
                              variant='rounded'
                              className='font-medium mie-2 text-white bg-[#2B3380] bs-[30px] is-12'
                            >
                              {slide.details[key]}
                            </CustomAvatar>
                            <Typography noWrap className='text-gray-700'>
                              {key}
                            </Typography>
                          </div>
                        </Grid>
                  ...
                  <Badge
                    ...
                    sx={{
                      '& .MuiBadge-dot': {
                        width: '8px !important',
                        height: '8px !important',
                        backgroundColor: '#2B3380 !important', // dot color changed
                        opacity: 0.4
                      },
                      '&.active .MuiBadge-dot': {
                        opacity: 1
