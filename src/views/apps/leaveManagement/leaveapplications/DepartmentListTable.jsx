...
import { updateLeaveApplication, fetchLeaveApplication } from "../../../../app/server/actions"
...
const DepartmentListTable = ({ tableData }) => {
  ...
  const refreshDepartments = async () => {
    ...
    const res = await fetchLeaveApplication(token)
    setData(res)
    setFilteredData(res)
  }
  ...
  const handleUpdateDepartment = async updatedData => {
    try {
      ...
      const response = await updateLeaveApplication(updatedData, token)
  ...
  () => [
    ...
    columnHelper.accessor('startDate', {
      header: 'Start Date',
       enableSorting: true,
      cell: ({ row }) => row.original.startDate?.split('T')[0] || '-'
    }),
    ...
    columnHelper.accessor('endDate', {
      header: 'End Date',
       enableSorting: true,
      cell: ({ row }) => row.original.endDate?.split('T')[0] || '-'
    }),
    ...
    columnHelper.accessor('appliedOn', {
      header: 'Applied On',
       enableSorting: true,
      cell: ({ row }) => row.original.appliedOn?.split('T')[0] || '-'
    }),
