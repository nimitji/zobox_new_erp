...
import { fetchCountNapsNats } from '../../../../app/server/actions'
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        ...
        // ⭐ API call with token
        const response = await fetchCountNapsNats(token)
        console.log('Fetched naps/nats data ✅', response)

        setCardsData(response)
      } catch (error) {
        console.error('Error fetching employee naps/nats ❌', error)
      }
    }
